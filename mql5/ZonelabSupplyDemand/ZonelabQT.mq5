//+------------------------------------------------------------------+
//|                                                       ZonelabQT.mq5|
//|  ZonelabSD ditambah gerbang waktu Quarterly Theory.                |
//|  Entry, stop, target, lot: IDENTIK dengan ZonelabSD. Satu satunya  |
//|  perbedaan adalah gerbang jam di depan order. Dengan keempat       |
//|  filter kosong dan InpRequireHighProb=0, EA ini HARUS memberi      |
//|  hasil yang sama persis dengan ZonelabSD, dan lengan itulah        |
//|  kontrolnya.                                                       |
...
input int    InpMagic            = 20260905;
//--- gerbang Quarterly Theory ---
// String kosong = tidak memfilter. "3" = hanya kuarter 3. "13" = 1 dan 3.
// 0 = tidak memfilter. 3 = hanya kuarter 3. 13 = kuarter 1 dan 3.
input int    InpWeeklyQuarters   = 0;   // 1=Sen 2=Sel 3=Rab 4=Kam
input int    InpDailyQuarters    = 0;   // 1=Asia 2=London 3=NY AM 4=NY PM
input int    InpQ90Quarters      = 0;   // kuarter 90 menit di dalam sesi
input int    InpRequireHighProb  = 0;   // 1 = rantai W-D-90m harus satu dari sepuluh
...
int g_zones_blocked_qt = 0;

// GERBANGNYA DINILAI DI BAR KEPUTUSAN, DAN ITU BATAS METODE YANG DINYATAKAN.
// Order yang dipasang di sini adalah LIMIT dan GTC, jadi ia bisa terisi
// berjam jam kemudian ketika kuarternya sudah berganti. Yang diukur lengan
// ini adalah "kapan setup boleh DIAMBIL", bukan "kapan ia terisi". Membuat
// gerbangnya mengikat di waktu isi butuh kedaluwarsa pending di ujung
// kuarter, dan itu mengubah tingkat isi sekaligus, jadi dua efek dalam satu
// angka. Satu lengan, satu efek.
bool QTGateOpen(datetime at)
  {
   if(!QTDigitAllowed(InpWeeklyQuarters, QTWeeklyQuarter(at)))
      return false;
   if(!QTDigitAllowed(InpDailyQuarters, QTDailyQuarter(at)))
      return false;
   if(!QTDigitAllowed(InpQ90Quarters, QTQuarter90(at)))
      return false;
   if(InpRequireHighProb != 0 && !QTHighProbChain(at))
      return false;
   return true;
  }
...
void OnDeinit(const int reason)
  {
   ...
   Print("zones blocked by QT gate: ", g_zones_blocked_qt);
   Print("QT filters: weekly=", InpWeeklyQuarters,
         " daily=", InpDailyQuarters,
         " q90=", InpQ90Quarters,
         " highprob=", InpRequireHighProb);
  }
   ...
   for(int i = 0; i < zcount; i++)
     {
      ...
      // TIDAK ADA MarkOrdered DI SINI, dan itu bukan kelalaian. Zona yang
      // ditolak jam harus tetap bisa diambil ketika kuarternya berganti;
      // menandainya akan mengubah gerbang waktu jadi penghapus zona permanen
      // dan lengan filter akan mengukur populasi yang lebih kecil karena
      // alasan yang salah.
      if(!QTGateOpen(time_[n-1]))
        {
         g_zones_blocked_qt++;
         continue;
        }
