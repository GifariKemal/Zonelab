//+------------------------------------------------------------------+
//|                                                       ZonelabBRK.mq5 |
//|  EA backtest breaker block di MT5 Strategy Tester.                 |
//|  Jalur trade-nya SALINAN PERSIS ZonelabFVG.mq5; yang berbeda cuma  |
//|  detektornya, supaya selisih hasilnya adalah selisih detektor.     |
//|                                                                    |
//|  DUA HAL YANG HARUS DIBACA SEBELUM ANGKANYA DIPAKAI.                |
//|                                                                    |
//|  1. ZONELAB TIDAK PUNYA JALUR ORDER UNTUK DETEKTOR INI.             |
//|     `tools/execute.py:192` hanya memanggil DETECTORS["supply_demand"],|
//|     dan `mark_profit_zones` tidak pernah dipanggil untuk imbalance   |
//|     mana pun, jadi setiap zona IFVG keluar dari Zonelab dengan       |
//|     profit_zone_rr = None dan target = None. Target profit_zone di   |
//|     bawah dipakai agar sebanding dengan ZonelabOB dan ZonelabFVG di  |
//|     matriks yang sama, BUKAN karena Zonelab menghitungnya.           |
//|                                                                    |
//|  2. ARAHNYA SUDAH DIUKUR DAN NEGATIF, dan untuk breaker angkanya    |
//|     yang paling buruk dari ketiganya. docs/CALIBRATION.md H8:       |
//|     order_block -0,274 (t=-4,22) lawan kontrol yang hanya tahu      |
//|     gerakan 20 bar terakhir, dan order_block juga negatif berdiri   |
//|     sendiri, -0,110 (t=-2,25) - kebalikan doktrin breaker block,    |
//|     bukan sekadar nolnya. EA ini ada untuk membuktikan port-nya     |
//|     presisi, bukan untuk mencari profit.                            |
...
input int    InpTargetMode       = 0;    // 0 = profit_zone, 1 = fixed R, 2 = fixed ATR
input double InpRewardR          = 2.0;
input double InpTargetAtr        = 2.0;
...
void DetectAndTrade()
  {
   ...
   SDZone zones[];
   int zcount=DetectBreaker(open_,high_,low_,close_,time_,atr,n,p,zones);
   ...
   for(int i=0;i<zcount;i++)
     {
      ...
      // `atr_base` sengaja TIDAK ikut berganti: ia juga memberi makan
      // target InpTargetAtr di bawah, dan menggeser dua hal sekaligus
      // membuat hasil sapuannya tidak bisa dibaca.
      ...
      if(InpTargetMode==1)
        ...
        {
         target=entry+way*InpTargetAtr*atr_base;
        }
      else
        {
      ...
      if(ok)
        ...
        {
         g_orders_failed++;
         Print("order gagal gap ",id,": ",trade.ResultRetcode());
        }
      MarkOrdered(id);
     }
  }
