...
class AurixProvider:
    """The local Aurix FastAPI instance, when it happens to be running.

    Aurix talks to a live MetaTrader 5 terminal, so this is the only provider
    here that returns the user's own broker's prices. It is optional by design:
    if MT5 is closed the app must still work.
    """
